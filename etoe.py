...
def encryption(plaintext: bytes, keyFileName: str, encryptedName: bytes) -> bytes:
    #global nonce_ctr
    nonce_ctr = 2**28
    ...
    ct = cipher.encrypt(nonce=bytes(str(nonce_ctr), 'ascii'),data=plaintext, associated_data=encryptedName)
    #nonce_ctr += 1
    return header, ct
